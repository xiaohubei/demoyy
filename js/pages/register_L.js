cb.views.register('RegisterViewController', function(controllerName) {
    ...
    view.prototype.init = function () {
        var proxy1 = cb.rest.DynamicProxy.create({ getCorpBaseMsg: { url: 'client/Corprations/getCorpBaseMsg', method: 'GET', } });
        proxy1.getCorpBaseMsg(function (err, result) {
            if (err) {

            } else {
                if (result.siteName) {
                    $('.ex_sigunp_titile').find('h1').text("欢迎来到" + result.siteName + ",期待您的加入!");
                }
        ...
        $("#getMessageAuthenticationCode").click(function() {
            if (phoneVerification() && view.prototype.phoneIsAble) {
                var proxy = cb.rest.DynamicProxy.create({ registerGetAuthCode: { url: 'member/Register/signUP_getAuthCode', method: 'POST', options: { token: true, mask: true } } });
                proxy.registerGetAuthCode({ 'phone': $('#mobile').val() }, function (err, result) {
                    if (err) {
                        alert(err.message);
                        return;
                    } else {
